...
# потом парсил Avito. это был бы самый полезный бот. для этого дела тоже нужна была lxml
# не сделал ещё и потому, что у них оказалась хорошая защита от парсинга (в принципе, это надо было ожидать с самого начала).


# проблема этого кода в том, что при отправке пользователем сообщений бот ломается. вылезают ошибки, обращаемые к строкам библиотек.
# т.е. ошибки рода "line 425", хотя здесь всего 43 строки. а что с ними делать - хз. наверняка есть ошибки в логике.
# или ошибки несовместимости версий библиотек. прост судя по тому, что я видел, этот же код работает на ура.
# а у меня во время runtime происходят ошибки, которые идут одна за другой, при этом выполнение программы не завершается.
